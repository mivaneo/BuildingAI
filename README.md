...
Final project for the Building AI course - Signaling messages analyzer for testing voice services in telecom world

## Summary

In the telecom world, there is a need to do tests to verify the implemented changes, the goal is that a machine can replace a human in certain actions. In order to verify the changes in the voice network, we need to perform E2E tests and then check if everything is as expected from user experience and from signaling perspective for each test. The user only needs to choose which test scenario he wants to test, and the rest is automated from execution of test scenario to the verification. We would use AI to do the verification part from signaling point of view with pass or fail result for each executed test scenario within the system environment.
...
We could use same automated solution for testing and analyzing data services within live/lab network.
